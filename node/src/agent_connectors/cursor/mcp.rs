...
fn parse_mcp_servers_object(mcp_servers: &Value, context_path: Option<String>) -> Vec<McpServer> {
...
fn parse_single_mcp_server(
    ...
    let full_command = match (command_str, args) {
        (Some(cmd), Some(args_arr)) => {
            let args_strs: Vec<&str> = args_arr.iter().filter_map(|a| a.as_str()).collect();
...
pub async fn discover_mcp_servers_from_configs(config_items: &[ConfigItem]) -> Vec<McpServer> {
    ...
    for item in config_items {
        ...
        match item.config_type.as_str() {
            //
            // Global settings file (~/.cursor/cli-config.json).
            // Format: { "mcpServers": { ... } }
            //
            ...
            //
            // Project MCP files (.cursor/mcp.json).
            ...
            config_type if config_type.starts_with("project_mcp:") => {
                ...
                match serde_json::from_str::<Value>(contents) {
                    Ok(json) => {
                        //
                        // Try top-level mcpServers key first.
                        //

                        if let Some(mcp_servers) = json.get("mcpServers") {
                            let parsed =
                                parse_mcp_servers_object(mcp_servers, context_path.clone());
                            if !parsed.is_empty() {
                                common::log_info!(
                                    "Found {} servers in project_mcp '{:?}' (mcpServers key)",
                                    parsed.len(),
                                    context_path
                                );
                                servers.extend(parsed);
                        ...
                        //
                        // Otherwise treat root object as server definitions.
                        //

                        if let Some(obj) = json.as_object() {
                            let mut count = 0;
                            for (name, server_config) in obj {
                                if let Some(server) = parse_single_mcp_server(
                                    name,
                                    server_config,
                                    context_path.clone(),
                                ) {
            ...
            //
            // Project settings files (.cursor/cli.json).
            // Format: { "mcpServers": { ... } }
            ...
            config_type if config_type.starts_with("project_settings:") => {
                ...
                let context_path = config_type
                    .strip_prefix("project_settings:")
                    .map(String::from);
                match serde_json::from_str::<Value>(contents) {
                    Ok(json) => {
                        if let Some(mcp_servers) = json.get("mcpServers") {
    ...
    common::log_info!("Parsed {} MCP servers from config files", servers.len());
