...
use crate::messages::{SkynetPlan, PlanStep, PlanStepStatus};
use crate::state::AppState;
use crate::rabbitmq::RabbitMqClient;

#[derive(Default)]
struct SkynetContext {
    selected_node_idx: usize,
    selected_agent_idx: usize,
}

/// Events from the Skynet handler
#[derive(Debug, Clone)]
pub enum SkynetEvent {
...
/// Skynet session state
pub struct SkynetSession {
...
pub fn get_system_prompt(custom_prompt: Option<&str>) -> Option<String> {
    ...
    if let Some(prompt) = custom_prompt {
        ...
        if !prompt.is_empty() {
            common::log_info!("Using custom skynet prompt ({} chars)", prompt.len());
            return Some(prompt.to_string());
        }
    }

    common::log_info!("No skynet prompt configured");
    None
}
...
async fn get_selected_node_id(app_state: &Arc<AppState>, ctx: &SkynetContext) -> Option<String> {
    if let Some(system_state) = app_state.get_state().await {
...
pub async fn start_skynet_session(
    app_state: Arc<AppState>,
    rabbitmq: Arc<RabbitMqClient>,
    _config_manager: Arc<crate::config::ConfigManager>,
    event_tx: mpsc::Sender<SkynetEvent>,
) -> Result<SkynetSession, String> {
    //
    ...
    //
    // Parse model definitions and find the selected Skynet model.
    //
    let model_defs_json = config.get("llm_model_definitions").cloned().unwrap_or_else(|| "[]".to_string());
    let selected_model_name = config.get("llm_feature_skynet").cloned().unwrap_or_default();
    ...
    let selected_def = model_defs.iter().find(|d| d.name == selected_model_name)
        .ok_or_else(|| format!("No model selected for Skynet. Go to Settings > LLM Providers > Feature Selection to configure."))?;

    let api_key = selected_def.api_key.clone();
    let provider_str = selected_def.provider.clone();
    let model = selected_def.model.clone();
    let custom_prompt = config.get("llm_skynet_prompt").cloned();
    let max_tokens: u32 = config.get("llm_skynet_max_tokens")
    ...
    let base_prompt = get_system_prompt(custom_prompt.as_deref())
        .ok_or_else(|| "No system prompt configured. Go to Settings > LLM Providers > Skynet to configure the system prompt.".to_string())?;
    let tools = get_tool_definitions();
    let system_prompt = get_system_prompt_with_tools(&base_prompt, &tools);

    common::log_info!("Skynet session starting with provider {:?}, model {}, max_tokens {}, history_count {}", provider, model, max_tokens, history_count);
    ...
    let task_handle = tokio::spawn(async move {
        ...
        let mut ctx = SkynetContext::default();
        ...
        while let Some(prompt) = prompt_rx.recv().await {
            ...
            common::log_info!("Skynet received prompt: {}...", &prompt[..prompt.len().min(50)]);
            ...
            loop {
                ...
                let (full_response, usage) = match client.chat_completion(request).await {
                    ...
                    Err(e) => {
                        ...
                        let _ = event_tx.send(SkynetEvent::Error(err_msg)).await;
                ...
                if let Some(usage) = usage {
                    let _ = event_tx.send(SkynetEvent::TokenUsage {
                ...
                while let Some((tool_name, tool_args, remaining_text)) = parse_manual_tool_call(&response_text) {
                    ...
                    common::log_info!("Skynet executing tool: {}", tool_name);
                    ...
                    let _ = event_tx.send(SkynetEvent::ToolExecuting { name: tool_name.clone(), input: tool_input_display }).await;
                    ...
                    if tool_name == "report_plan" {
                        if let Ok(result_json) = serde_json::from_str::<Value>(&result) {
                            if let Some(plan_obj) = result_json.get("plan") {
                                if let Ok(plan) = serde_json::from_value::<SkynetPlan>(plan_obj.clone()) {
                                    let _ = event_tx.send(SkynetEvent::PlanUpdated(plan)).await;
                                }
                            }
                        }
                    }

                    let _ = event_tx.send(SkynetEvent::ToolExecuted {
                ...
                if !tool_results.is_empty() {
                    ...
                    if !remaining.is_empty() {
                        let _ = event_tx.send(SkynetEvent::Content(remaining.to_string())).await;
                    }
                ...
                if !full_response.is_empty() {
                    let _ = event_tx.send(SkynetEvent::Content(full_response.clone())).await;
                }
            ...
            let _ = event_tx.send(SkynetEvent::Done).await;
        }
    });
